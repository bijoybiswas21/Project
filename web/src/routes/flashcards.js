...
flashcardRoutes.get("/review", (req, res) => {
...
flashcardRoutes.post("/", (req, res) => {
  ...
  const { lastId } = execute("INSERT INTO flashcards (user_id, subject_id, question, answer) VALUES (?, ?, ?, ?)",
    [req.userId, Number(subject_id), question.trim(), answer.trim()]);
  res.status(201).json({ id: lastId });
});

flashcardRoutes.put("/:id/review", (req, res) => {
  const { quality } = req.body;
  const id = Number(req.params.id);
  const card = queryOne("SELECT * FROM flashcards WHERE id = ? AND user_id = ?", [id, req.userId]);
  ...
  execute(`UPDATE flashcards SET difficulty = ?, next_review = datetime('now', '+' || ? || ' days') WHERE id = ? AND user_id = ?`,
    [rounded, intervalDays, id, req.userId]);
  res.json({ success: true, next_review_days: intervalDays });
});

flashcardRoutes.delete("/:id", (req, res) => {
  execute("DELETE FROM flashcards WHERE id = ? AND user_id = ?", [Number(req.params.id), req.userId]);
