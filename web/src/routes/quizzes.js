...
quizRoutes.get("/:id", (req, res) => {
  ...
  quiz.questions = queryAll("SELECT * FROM quiz_questions WHERE quiz_id = ?", [id]);
  res.json(quiz);
});
...
quizRoutes.post("/", (req, res) => {
  ...
  const { lastId: quizId } = execute("INSERT INTO quizzes (user_id, subject_id, title) VALUES (?, ?, ?)",
  ...
  for (const q of questions) {
    ...
    execute(`INSERT INTO quiz_questions
...
quizRoutes.post("/:id/attempt", (req, res) => {
  const quizId = Number(req.params.id);
  const quiz = queryOne("SELECT id FROM quizzes WHERE id = ? AND user_id = ?", [quizId, req.userId]);
  if (!quiz) return res.status(404).json({ error: "Quiz not found" });
  const { answers } = req.body;
  const questions = queryAll("SELECT * FROM quiz_questions WHERE quiz_id = ?", [quizId]);
  ...
  execute("INSERT INTO quiz_attempts (user_id, quiz_id, score, total) VALUES (?, ?, ?, ?)",
    [req.userId, quizId, score, questions.length]);
  res.json({ score, total: questions.length, results });
});

quizRoutes.delete("/:id", (req, res) => {
  execute("DELETE FROM quizzes WHERE id = ? AND user_id = ?", [Number(req.params.id), req.userId]);
