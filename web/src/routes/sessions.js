...
sessionRoutes.post("/", async (req, res) => {
  ...
  const { lastId } = await execute("INSERT INTO study_sessions (user_id, subject_id, duration_minutes, notes) VALUES (?, ?, ?, ?)",
...
sessionRoutes.delete("/:id", async (req, res) => {
  await execute("DELETE FROM study_sessions WHERE id = ? AND user_id = ?", [Number(req.params.id), req.userId]);
