...
noteRoutes.post("/", async (req, res) => {
  ...
  const { lastId } = await execute("INSERT INTO notes (user_id, subject_id, title, content) VALUES (?, ?, ?, ?)",
...
noteRoutes.put("/:id", async (req, res) => {
  ...
  await execute(`UPDATE notes SET
...
noteRoutes.delete("/:id", async (req, res) => {
  await execute("DELETE FROM notes WHERE id = ? AND user_id = ?", [Number(req.params.id), req.userId]);
  res.json({ success: true });
});
